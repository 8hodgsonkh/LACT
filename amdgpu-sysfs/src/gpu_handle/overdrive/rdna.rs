...
pub struct Table {
    ...
    /// Per-zone voltage offsets (in mV) for V/F curve tuning.
    /// Available on GPUs with per-zone voltage offset support (e.g. RDNA3 with kernel patches).
    #[cfg_attr(feature = "serde", serde(default, skip_serializing_if = "Vec::is_empty"))]
    pub voltage_offset_per_zone: Vec<i32>,
    /// Per-zone frequency ranges (low_MHz, high_MHz) corresponding to each voltage offset zone.
    #[cfg_attr(feature = "serde", serde(default, skip_serializing_if = "Vec::is_empty"))]
    pub voltage_offset_zone_freq_ranges: Vec<(u32, u32)>,
...
impl ClocksTable for Table {
    fn write_commands<W: Write>(
        ...
        if !self.voltage_offset_per_zone.is_empty() {
            for (zone, offset) in self.voltage_offset_per_zone.iter().enumerate() {
                let line = format!("vo {} {}\n", zone, offset);
                writer
                    .write_all(line.as_bytes())
                    .with_context(|| format!("Error when writing per-zone voltage offset `{line}`"))?;
            }
        } else if let Some(offset) = self.voltage_offset {
...
impl Table {
    ...
    /// Sets a per-zone voltage offset, checking if it's in the allowed range
    pub fn set_voltage_offset_per_zone(&mut self, zone: usize, offset: i32) -> Result<()> {
        if let Some(offset_range) = self.od_range.voltage_offset {
            if let Some((min, max)) = offset_range.into_full() {
                if !(min..=max).contains(&offset) {
                    return Err(Error::not_allowed(format!(
                        "Provided voltage offset {offset} for zone {zone} is out of range, should be between {min} and {max}"
                    )));
                }
            }
        }
        while self.voltage_offset_per_zone.len() <= zone {
            self.voltage_offset_per_zone.push(0);
        }
        self.voltage_offset_per_zone[zone] = offset;
        Ok(())
    }
}
...
impl FromStr for Table {
    ...
    fn from_str(s: &str) -> Result<Self> {
        ...
        let mut voltage_offset_per_zone = Vec::new();
        let mut voltage_offset_zone_freq_ranges = Vec::new();
        ...
        while let Some(line) = lines.next() {
            match line {
                ...
                "OD_VF_CURVE_OFFSETS:" => current_section = Some(Section::VfCurveOffsets),
                line => match current_section {
                    ...
                    Some(Section::VfCurveOffsets) => {
                        let (offset, freq_range) = parse_indexed_voltage_offset_line(line, i)?;
                        voltage_offset_per_zone.push(offset);
                        if let Some(range) = freq_range {
                            voltage_offset_zone_freq_ranges.push(range);
                        }
                    }
                    None => {
                        return Err(ParseError {
        ...
        Ok(Self {
            ...
            od_range,
            voltage_offset,
            voltage_offset_per_zone,
            voltage_offset_zone_freq_ranges,
        })
    }
}
...
impl Table {
    ...
    pub fn clear(&mut self) {
        ...
        self.voltage_offset_per_zone.clear();
        self.voltage_offset_zone_freq_ranges.clear();
    }
...
fn parse_indexed_voltage_offset_line(line: &str, i: usize) -> Result<(i32, Option<(u32, u32)>)> {
    ...
    match line.split_once(':') {
        Some((_, value_part)) => {
            ...
            let offset = parse_voltage_offset_line(mv_part.trim(), i)?;
            Ok((offset, freq_range))
        }
        None => Err(ParseError {
            msg: format!("Could not parse indexed voltage offset line: {line}"),
            line: i,
        }
        .into()),
    }
...
mod tests {
    ...
    fn write_commands_custom_5700xt() {
        let table = Table {
            ...
            voltage_offset: None,
            voltage_offset_per_zone: Vec::new(),
            voltage_offset_zone_freq_ranges: Vec::new(),
            od_range: OdRange {
                sclk: None,
                sclk_offset: None,
                mclk: None,
